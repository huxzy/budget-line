 ...
 */
export function VoiceOverlay({ session, data, onClose }: { session: VoiceSession; data: AtlasData; onClose: () => void }) {
  const router = useRouter();
  const heard = session.partial?.role === "user" ? session.partial.text : [...session.lines].reverse().find((l) => l.role === "user")?.text ?? "";
  const last = session.results[session.results.length - 1]?.payload as Record<string, unknown> | undefined;
  ...
  return (
    <div className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-6 bg-clay/55 p-6 motion-safe:animate-veil" role="dialog" aria-modal="true" aria-label="Say a place">
      <div className="flex h-8 items-end gap-1" aria-hidden>
        {[0.4, 0.7, 1, 0.6, 0.8].map((h, i) => (
          <span key={i} className="w-1.5 rounded-full bg-on-clay/80 motion-safe:animate-wave" style={{ height: `${h * 100}%`, animationDelay: `${i * 90}ms` }} />
        ))}
      </div>
        ...
        <div className="flex w-full max-w-[440px] items-center gap-4 rounded-[18px] bg-surface p-5 shadow-hero motion-safe:animate-rise">
          <div className="min-w-0 flex-1">
            <span className="eyebrow">
              {match.kind === "lga" ? `Local government · ${match.stateName} State` : "State"}
            </span>
