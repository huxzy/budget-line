...
export default async function AreaPage({ params }: { params: Promise<{ state: string; lga: string }> }) {
  ...
  return (
    <Suspense>
      <AskScreen voice={voiceConfigFor(language.code, registry.slug)} registry={registry} places={lgas.map((l) => ({ lga: l.lga, lgaLabel: l.lgaLabel }))} languageName={language.name} area={{ lga: area.lga, lgaLabel: area.lgaLabel }} />
    </Suspense>
  );
}
